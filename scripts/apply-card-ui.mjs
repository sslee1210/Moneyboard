...
if(existsSync(app)){let s=readFileSync(app,'utf8');const fn=String.raw`function SectorRow({ sector, selected, maxValue, onSelect }) {
  ...
  const topStocks = [...stockSource].sort((a, b) => ((b.tradeAmountMillion || b.tradingValueMillion || 0) - (a.tradeAmountMillion || a.tradingValueMillion || 0)) || ((b.volume || 0) - (a.volume || 0))).slice(0, 5);
  return (
    <button className={\`sector-row sector-card-block ${selected ? "is-selected" : ""}\`} onClick={() => onSelect(sector)}>
      <div className="sector-card-topline"><span className="sector-rank">{sector.rank}</span><span className={\`sector-change ${changeClass(changeRate)}\`}>{changeRate >= 0 ? <ArrowUpRight size={14} /> : <ArrowDownRight size={14} />}{formatPercent(changeRate)}</span></div>
      <div className="sector-card-titleline"><strong>{sector.name}</strong><small>{sector.stockCount || 0}종목 · {sector.topStockName || topStocks[0]?.name || "집계 중"}</small></div>
      <div className="sector-card-moneyline"><span>거래대금</span><strong>{formatTradingValue(sector.tradingValueMillion)}</strong></div>
      ...
      <div className="sector-card-stocklist">
        {topStocks.map((stock, index) => {
          ...
          return <span key={stock.code || \`${stock.name}-${index}\`}><em>{index + 1}</em><strong title={stock.name}>{stock.name}</strong><small>{formatTradingValue(money)}</small></span>;
        })}
...
s=s.replace(/function SectorRow\(\{ sector, selected, maxValue, onSelect \}\) \{[\s\S]*?\n\}\n\nfunction stockPeriodVolume/,`${fn}\n\nfunction stockPeriodVolume`);writeFileSync(app,s,'utf8');console.log('[card-ui] sector card block UI applied');}
const css=file('src/styles.css');
if(existsSync(css)){let s=readFileSync(css,'utf8');if(!s.includes('/* card-block-ui */')){s+=String.raw`
...
`;writeFileSync(css,s,'utf8');console.log('[card-ui] css appended');}else console.log('[card-ui] css already present');}
